...
global_variable GLfloat GlobalPointSize = 0.0f;
global_variable GLfloat GlobalPointSizes[2] = { 0.0f, 0.0f };
global_variable GLfloat GlolbalPointGranularity = 0.0f;
...
OpenGLRender(float angle)
{
    ...
    GlobalPointSize = 0.5f;
    for (float x = -4.0f; x < 5.0f; x += 0.5f)
    {
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    // NOTE(nick): test code
    /*
    // check supported point size level of antialiasing implementation
    glGetFloatv(GL_POINT_SIZE_RANGE, GlobalPointSizes);
    GLfloat minPointSize = GlobalPointSizes[0];
    GLfloat maxPointSize = GlobalPointSizes[1];

    // check current point size
    glGetFloatv(GL_POINT_SIZE, &GlobalPointSize);
    if (GlobalPointSize > maxPointSize)
    {
        GlobalPointSize = maxPointSize;
    }
    glPointSize(GlobalPointSize);

    // retrieves point size granularity
    glGetFloatv(GL_POINT_SIZE_GRANULARITY, &GlolbalPointGranularity);

    // check if point antialiasing is currently disabled, and enable it
