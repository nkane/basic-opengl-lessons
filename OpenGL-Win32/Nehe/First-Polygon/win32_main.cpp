...
int DrawGLScene(GLvoid)
{
	...
	// move view left 1.5 units and into the screen 6.0
	glTranslatef(-1.5f, 0.0f, -6.0f);

	// draw using triangles
	glBegin(GL_TRIANGLES);
	{
		// NOTE: glVertex3f(x_coordinate, y_coordinate, z_coordinate)
		// top
	...
	// move view right 3 units 
	glTranslatef(3.0f, 0.0f, 0.0f);

	// NOTE: 
	// a) objects drawn clockwise will be drawn as a back face
	// b) objects drawn counter clockwise will be drawn facing the viewer.
	// draw using quadrilaterals
	glBegin(GL_QUADS);
	{
		// top left
		glVertex3f(-1.0f, 1.0f, 0.0f);
