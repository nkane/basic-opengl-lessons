...
Calculate_Directional_Light(Directional_Light light, vec3 normal, vec3 view_direction)
{
    ...
    // diffuse shading
    float diff = max(dot(normal, light_direction));
    // specular shading
...
main()
{
    ...
    {
        result += Calculate_Point_Light(point_lights[i], norm, v_fragment_position, view_direction);
    }
    // phase 3: spot light
    //result += Calculate_Spot_Light(sport_light, norm, v_fragment_position, view_direction);
