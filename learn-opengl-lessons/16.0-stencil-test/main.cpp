...
main()
{
    ...
    // NOTE(nick): depth test 
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);

    // NOTE(nick): stencil test 
    ...
    ShaderProgram *stencil_shader = CreateShaderProgram("./shaders/stencil_vertex.glsl", "./shaders/stencil_fragment.glsl");
    if (!stencil_shader)
    {
        glfwTerminate();
        return -1;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        SetFloatMat4Uniform(stencil_shader, "view", glm::value_ptr(view));
        SetFloatMat4Uniform(stencil_shader, "projection", glm::value_ptr(projection));
        ...
        // 1st render pass, draw objects as normal, write to the stencil buffer
        ...
        // this time disabling stencil writing, because the stencil buffer is
        // now filled with several 1s, the parts of the buffer that are 1 are
        // not drawn, thus only drawing the objects' size differences, making
        // it look like borders
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilMask(0x00);
        ...
        SetFloatMat4Uniform(stencil_shader, "model", glm::value_ptr(model));
        glDrawArrays(GL_TRIANGLES, 0, 36);
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
        model = glm::scale(model, glm::vec3(scale, scale, scale));
    ...
    // TODO(nick): delete things / clean stuff up
