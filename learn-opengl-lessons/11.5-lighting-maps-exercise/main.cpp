/*
 *  Exercise 4
 *  - Also add something they call an emission map which is a texture
 *    that stores emission values per fragment. Emission values are
 *    colors an object may emit as if it contains a light source itself;
 ...
 *    (like eyes of a robot, or light strips on a container). Add the
 *    following texture (by creativesam) as an emission map onto the
 *    container as if the letters emit light.
 */
...
main()
{
    ...
    Texture emission_map_texture = {};
    stbi_set_flip_vertically_on_load(true);
    emission_map_texture.data = stbi_load("assets/matrix.jpg", &emission_map_texture.width, &emission_map_texture.height, &emission_map_texture.channels, 0);
    if (!emission_map_texture.data)
    {
        printf("[STB_Image ERROR]: failed to load image\n");
        return -1;
    }
    glGenTextures(1, &emission_map_texture.id);
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, emission_map_texture.id);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, emission_map_texture.width, emission_map_texture.height, 0, GL_RGB, GL_UNSIGNED_BYTE, emission_map_texture.data);
    glGenerateMipmap(GL_TEXTURE_2D);
    stbi_image_free(emission_map_texture.data);
    ...
    SetIntUniform(shader_program, "u_material.emission", 2);
