...
main()
{
    ...
    const char *grass_path = "assets/grass.png";
    stbi_set_flip_vertically_on_load(false);
    grass_texture.data = stbi_load(grass_path,
                          &grass_texture.width,
                          &grass_texture.height,
                          &grass_texture.channels, 0);
    if (!grass_texture.data)
    {
        printf("[STB_Image ERROR]: failed to load image (%s)\n", grass_path);
        return -1;
    }
    switch (grass_texture.channels)
    {
        case 1:
        {
            format = GL_RED;
    ...
    glGenTextures(1, &grass_texture.id);
    glBindTexture(GL_TEXTURE_2D, grass_texture.id);
    glTexImage2D(GL_TEXTURE_2D, 0, format, grass_texture.width, grass_texture.height, 0, format, GL_UNSIGNED_BYTE, grass_texture.data);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glGenerateMipmap(GL_TEXTURE_2D);
    stbi_image_free(grass_texture.data);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            glm::mat4 veg_model = glm::mat4(1.0f);
            veg_model = glm::translate(veg_model, vegetation[i]);
            SetFloatMat4Uniform(shader,"model", glm::value_ptr(veg_model));
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
