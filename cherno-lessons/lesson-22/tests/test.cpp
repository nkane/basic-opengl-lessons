...
Create_Test_Quad()
{
    Test_Quad *Result = NULL;
    Result = (Test_Quad *)malloc(sizeof(Test_Quad));
    if (Result == NULL)
    {
        return NULL;
    }

    Result->Vertices[0] = -0.5f;
    Result->Vertices[1] = -0.5f;
    Result->Vertices[2] =  0.0f;

    Result->Vertices[3] =  0.5f;
    Result->Vertices[4] = -0.5f;
    Result->Vertices[5] =  0.0f;

    Result->Vertices[6] =  0.5f;
    Result->Vertices[7] =  0.5f;
    Result->Vertices[8] =  0.0f;

    Result->Vertices[9] = -0.5f;
    Result->Vertices[10] =  0.5f;
    Result->Vertices[11] =  0.0f;

    Result->VertexArray = NewVertexArray();
    ASSERT(Result->VertexArray != NULL);

    Result->VertexBuffer = NewVertexBuffer(Result->Vertices, 12 * sizeof(float));
    ASSERT(Result->VertexBuffer != NULL);

    Result->VertexBufferLayout = NewVertexBufferLayout(2);
    ASSERT(Result->VertexBufferLayout != NULL);
    Push(Result->VertexBufferLayout, GL_FLOAT, 3);

    AddBuffer(Result->VertexArray, Result->VertexBuffer, Result->VertexBufferLayout);
    ...
    Result->IndexBuffer = NewIndexBuffer(Result->Indices, 6);
    ASSERT(Result->IndexBuffer != NULL);
